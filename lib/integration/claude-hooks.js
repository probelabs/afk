...
class ClaudeHooksService {
  ...
  async generatePreviewDiffImage(toolName, toolInput, cwd) {
    try {
      ...
      this.logger.eprint(`[afk] Error generating preview diff image: ${error.message}`);
      return null;
    }
  }
  ...
  async generateBeautifulDiffFromContent(diffContent, title = 'Preview of Proposed Changes') {
    try {
      ...
      try {
        ...
        this.logger.eprint(`[afk] Error running diff generation script: ${scriptError.message}`);
      } finally {
        // Clean up temp file
        try {
      ...
      this.logger.eprint(`[afk] Error generating beautiful preview diff: ${error.message}`);
      return null;
    }
  }
  ...
  async generateDiffImage(cwd) {
    try {
      ...
      const diff = execSync('git diff', { cwd, encoding: 'utf8', maxBuffer: 100000 }).trim();
      if (!diff) {
        this.logger.eprint('[afk] No git changes found, skipping diff image generation');
        return null;
      }
      ...
      this.logger.eprint(`[afk] Error generating diff image: ${error.message}`);
      return null;
    }
  }
  ...
  generateFormattedDiff(cwd) {
    try {
      ...
      this.logger.eprint(`[afk] Error generating formatted diff: ${error.message}`);
      return null;
    }
  }
